...
async function pagi(Lyrra, x, data) {
    ...
    return await Lyrra.sendMessage(x.room, {
        text: pagii(x),
        contextInfo: {
            externalAdReply: {
...
async function voting(Lyrra, x, data) {
    let row = [];
    let ment = [];
...
async function malam(Lyrra, x, data) {
    ...
    if (hasil_vote === 0) {
        ...
        return Lyrra.sendMessage(x.room, {
                text: textnya,
                contextInfo: {
                    externalAdReply: {
                        title: "W E R E W O L F",
...
async function skill(Lyrra, x, data) {
    skillOn(x.room, data)
        ...
        for (let i = 0; i < x.player.length; i++) {
            if (x.player[i].role === "werewolf") {
                if (x.player[i].isdead != true) {
                    ...
                    await Lyrra.sendMessage(x.player[i].id, {
                        text: textnya,
                        mentions: membernya,
                    });
                }
...
async function win(x, t, Lyrra, data) {
    ...
    if (getWinner(x.room, data).status === false || x.iswin === false) {
        ...
        return await Lyrra.sendMessage(sesinya, {
            text: textnya,
            contextInfo: {
                externalAdReply: {
                    title: "W E R E W O L F",
...
async function run(Lyrra, id, data) {
    while (getWinner(id, data).status === null) {
        ...
        if (getWinner(id, data).status != null) {
            ...
            await pagi(Lyrra, sesi(id, data), data);
        }
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, Lyrra, data);
            break;
        ...
        if (getWinner(id, data).status != null) {
            ...
            await voting(Lyrra, sesi(id, data), data);
        }
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, Lyrra, data);
            break;
        ...
        if (getWinner(id, data).status != null) {
            ...
            await malam(Lyrra, sesi(id, data), data);
        }
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, Lyrra, data);
            break;
        } else {
            await skill(Lyrra, sesi(id, data), data);
        }
        if (getWinner(id, data).status != null) break;
    }
    await win(sesi(id, data), 1, Lyrra, data);
}

async function run_vote(Lyrra, id, data) {
    while (getWinner(id, data).status === null) {
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, Lyrra, data);
            break;
...
async function run_malam(Lyrra, id, data) {
    while (getWinner(id, data).status === null) {
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, Lyrra, data);
            break;
...
async function run_pagi(Lyrra, id, data) {
    while (getWinner(id, data).status === null) {
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, Lyrra, data);
            break;
