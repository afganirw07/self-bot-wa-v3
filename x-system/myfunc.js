...
exports.smsg = (Lyrra, m, store) => {
    ...
    if (m.key) {
        ...
        m.sender = Lyrra.decodeJid(m.fromMe && Lyrra.user.id || m.participant || m.key.participant || m.chat || '')
        if (m.isGroup) m.participant = Lyrra.decodeJid(m.key.participant) || ''
    }
    if (m.message) {
    ...
    : m.message?.conversation || 
      ...
      (m.mtype === 'viewOnceMessageV2' && (m.msg.message?.imageMessage?.caption || m.msg.message?.videoMessage?.caption)) || ''
        ...
        if (m.mtype == "viewOnceMessageV2" || m.msg.url) m.download = () => Lyrra.downloadMediaMessage(m)
        m.copy = () => exports.smsg(Lyrra, M.fromObject(M.toObject(m)))
        m.copyNForward = (jid = m.chat, forceForward = false, options = {}) => Lyrra.copyNForward(jid, m, forceForward, options)
        m.reply = (text, chatId = m.chat, options = {}) => Buffer.isBuffer(text) ? Lyrra.sendMedia(chatId, text, 'file', '', m, { ...options }) : Lyrra.sendText(chatId, text, m, { ...options })
    ...
    (m.quoted.id.startsWith('N5G9') && m.quoted.id.length === 24) : false;
            m.quoted.sender = Lyrra.decodeJid(m.msg.contextInfo.participant)
            m.quoted.fromMe = m.quoted.sender === (Lyrra.user && Lyrra.user.jid)
            ...
            m.quoted.download = () => Lyrra.downloadMediaMessage(m.quoted)
            m.quoted.copyNForward = (jid, forceForward = false, options = {}) => Lyrra.copyNForward(jid, vM, forceForward, options)
            m.getQuotedObj = m.getQuotedMessage = async () => {
                if (!m.quoted.id) return false
                let q = await store.loadMessage(m.chat, m.quoted.id, Lyrra)
                return exports.smsg(Lyrra, q, store)
            }
    ...
    m.name = Lyrra.getName(m.sender)
    if (m.msg.url) m.download = () => Lyrra.downloadMediaMessage(m.msg)
    m.reply = (text, chatId = m.chat, options = {}) => Buffer.isBuffer(text) ? Lyrra.sendMedia(chatId, text, 'file', '', m, { ...options }) : Lyrra.sendText(chatId, text, m, { ...options })
    m.copyNForward = (jid = m.chat, forceForward = false, options = {}) => Lyrra.copyNForward(jid, m, forceForward, options)
    return m
}
